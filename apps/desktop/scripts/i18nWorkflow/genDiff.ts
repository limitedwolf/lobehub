...
export const genDiff = () => {
  consola.start(`Comparing locale files between dev and prod environments...`);
  ...
  for (const [ns, devJSON] of data) {
    ...
    if (!existsSync(filepath)) {
      consola.info(`File not found, skipping: ${filepath}`);
      continue;
    }
    ...
    if (clearLocals.length > 0) {
      consola.info('Cleaned up stale keys for locales:', clearLocals.join(', '));
    }
    consola.success(tagWhite(ns), colors.gray(filepath));
  }
};
