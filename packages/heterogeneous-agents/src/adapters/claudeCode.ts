...
export class ClaudeCodeAdapter implements AgentEventAdapter {
 ...
 // CC's long-running task lifecycle (Monitor, etc.).    // `task_started` registers a task that may fire callback turns;
 ...
 // the UI's StatusIndicator stuck on the spinner.                  if (c?.type === 'tool_reference' && c.tool_name) return c.tool_name;
