    ...
    switch (event.type) {
      ...
      case 'error': {
        enqueue(async () => {
          ...
          // An errored run is a FAILED run, not a completed one (error → fail).
          // For gateway, drive the terminal disposition through the shared
          // lifecycle so the op lands in `failed` (no unread badge, no queue
