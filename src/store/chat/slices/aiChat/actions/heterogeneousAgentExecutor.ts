 ...
 * Required for the **toolless middle step** case: when a step
  ...
  try {
    ...
    /**
 * Process a single `AgentStreamEvent` from main. As of phase 0     * main runs the adapter and `toStreamEvent` itself, so each IPC arrival
 ...
 // warnings even after the DB has settled.      // DB state is already correct (the subagent persist path writes to
