...
// keep their behaviour. Individual tests can replace this via
...
describe('BotMessageRouter', () => {
 ...
 // Sanity: existing call sites must keep their semantics.      const handler = await loadSubscribedHandler({ dmPolicy: 'open' });
      const thread = makeThread({ isDM: false });
